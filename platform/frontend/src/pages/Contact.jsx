...
const Contact = () => {
  ...
  const validate = () => {
    const err = {};
    if (!form.name.trim()) err.name = text.locale === 'zh' ? '请输入姓名' : 'Name is required';
    if (!form.email.trim()) err.email = text.locale === 'zh' ? '请输入邮箱' : 'Email is required';
    else if (!/^\S+@\S+$/i.test(form.email)) err.email = text.locale === 'zh' ? '邮箱格式不正确' : 'Invalid email';
    if (!form.message.trim()) err.message = text.locale === 'zh' ? '请输入内容' : 'Message is required';
    return err;
  };
    ...
    try {
      ...
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: text.locale === 'zh' ? '来自 Platform 网站的新消息' : 'New Message from Platform Website',
        }),
      });
      ...
      if (result.success) {
        ...
        setErrorMessage(result.message || (text.locale === 'zh' ? '发送失败。' : 'Failed to send message.'));
      }
    } catch (error) {
      console.error("Error sending email:", error);
      setErrorMessage(text.locale === 'zh' ? '发生错误，请稍后重试。' : 'Something went wrong. Please try again.');
    } finally {
      setLoading(false);
    }
  };
