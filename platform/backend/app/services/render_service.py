...
class RenderService:
    ...
    def render_slideshow(
        ...
        background_rgb: tuple[int, int, int] = (10, 10, 14),
    ) -> dict:
        ...
        try:
            for panel_path in panel_paths:
                frame = RenderService._build_frame(panel_path, frame_size, background_rgb)
    ...
    @staticmethod
    def _build_frame(panel_path: str, frame_size: tuple[int, int], background_rgb: tuple[int, int, int]):
        width, height = frame_size
        canvas = Image.new("RGB", (width, height), color=background_rgb)
