...
Before starting the frontend, create `platform/frontend/.env.local` from `platform/frontend/.env.example` and set the backend API URL.
...
3. Backend local auth env values are filled in `platform/backend/.env`:
...
4. Frontend auth values are filled in `platform/frontend/.env.local`:
